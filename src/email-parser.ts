...
export async function parseEmailForData(rawEmail: ReadableStream<Uint8Array>): Promise<ExtractedEmailData> {
  return new Promise((resolve, reject) => {
    ...
    parser.on('end', () => {
      ...
      const confirmationMatch = textContent.match(/Confirmation Code:\s*([A-Z0-9]+)/i);
